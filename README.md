...
- 引用可从回答卡片直接打开对应资料并显示页码、章节或 chunk 定位；OCR 字段和表格支持人工校正并重建检索索引。
...
- 命中监控规则后可发送 Chrome 插件通知、飞书、钉钉或通用 Webhook；通知失败不会把本次采集误判为失败。
...
扩展当前申请 `tabs`、`storage`、`scripting`、`activeTab`、`downloads`、`alarms`、`offscreen`、`notifications` 和 `sidePanel` 等权限。可选的 `debugger` 权限仅用于增强页面诊断，默认路径不依赖它。
...
- **V4 模板平台**：命令发布、模板审批、权限策略、运行指标和团队级同步。
