...
import { buildMemoryContext } from '../../../../shared/memory/userMemoryStore';
import { RUNTIME_BUILD_ID } from '../../../../shared/runtime/runtimeVersion';
import {
  getActiveBrowserTabId,
  shouldStopTypingForGatewayStatus,
  type GatewayConnectionStatus,
} from '../../../utils/chat/chatRequestState';
import {
  hasRenderableChatMessage,
  mergeIncomingChatMessage,
  shouldPersistIncomingChatMessage,
} from '../../../utils/chat/chatMessageState';
...
export interface AiRequestContextOptions {
  resolveAdditionalContext?: () => Promise<string | undefined>;
  onContextResolved?: (contextText: string) => void;
  onComplete?: () => void;
}
...
export function useAiRequest(options: {
  ...
  const sendUserMessage = useCallback((
    ...
    contextOptions: AiRequestContextOptions = {},
  ) => {
    ...
    const baseMessageHistory = updatedMessages
    ...
    Promise.all([
      ...
      getActiveBrowserTabId(),
      contextOptions.resolveAdditionalContext?.().catch((error) => {
        console.warn('[PageAwareChat] 页面上下文采集失败，降级为普通问答:', error);
        return undefined;
      }) || Promise.resolve(undefined),
    ])
      .then(([memoryContext, contextTabId, additionalContext]) => {
        const contextText = String(additionalContext || '').trim();
        if (contextText) contextOptions.onContextResolved?.(contextText);
        const messageHistory = baseMessageHistory.map((item, index) => (
          contextText && index === baseMessageHistory.length - 1 && item.role === 'user'
