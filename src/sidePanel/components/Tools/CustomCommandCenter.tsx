...
const CustomCommandCenter: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const openEditor = (command: CustomCopilotCommand | null) => {
    setEditing(command);
    form.setFieldsValue(command || { mode: 'prompt', riskLevel: 'low', enabled: true, taskKind: 'computer_use', metadataJson: '{}' });
  ...
  const save = async () => {
    ...
    await upsertCustomCommand({ ...editing, ...values, metadata });
