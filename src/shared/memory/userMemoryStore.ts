import type { PageSelectionContext } from '../context/pageContextActions';
...
export type ChatSession = {
  ...
  researchStatus?: 'collecting' | 'ready' | 'partial';
};
...
export async function createChatSession(
  input: Partial<Pick<
    ChatSession,
    'title' | 'summary' | 'tags' | 'mode' | 'coreQuestion' | 'sourceDocumentIds' | 'researchStatus'
  >> = {},
): Promise<ChatSession> {
  const now = Date.now();
  const session: ChatSession = {
    ...
    sourceDocumentIds: Array.from(new Set(input.sourceDocumentIds || [])),
